...
namespace MixPiece {
    ...
    // void __decompress_block(IterIO& file, int interval, time_t basetime, int length, float slope, float intercept) {
    ...
    //     }
    //     // Decompress part 1
    //     int i = compress_data->getInt();
    //     while (i-- > 0) {
    //         float b = compress_data->getFloat();
    //         int m = compress_data->getInt();
    //         while (m-- > 0) {
    ...
    //         __decompress_block(outputFile, interval, basetime, length, slope, intercept);
