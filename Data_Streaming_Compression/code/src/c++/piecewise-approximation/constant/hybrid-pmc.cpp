...
namespace HybridPMC {
    ...
    void _yield(BinObj* obj, int length, float value) {
        obj->put(length);
        obj->put(value);
    }

    void _approximate(IterIO& file, int interval, time_t basetime, int prev_point, int length, float value) {
    ...
    void _pmc(BinObj* obj, std::vector<float>& window, float bound) {
        float min = INFINITY;
        float max = -INFINITY;
        float value = 0;
        int length = 0;
            ...
            if (buffer.size() % w_size == 0) {
                if (max - min <= 2 * bound) {
                    if (buffer.size() == w_size*m_window) {
                        _yield(obj, buffer.size(), (max + min) / 2);
                        buffer.clear();
                        min = INFINITY;
                        max = INFINITY;
                    }
                    ...
                    if (buffer.size() > w_size) {
                        ...
                        _yield(obj, buffer.size(), (b_max + b_min) / 2);
                    }
                    buffer.clear();
